...
stem.polys.spdf
stem.polys.spdf@plotOrder
stem.polys.spdf@data
ID
polys$
...
library(plyr)
insitu_maxStemHeight <- ddply(SITE_insitu,.(plotid),summarize,max = max(stemheight,na.rm=T))
insitu_meanStemHeight <- ddply(SITE_insitu,.(plotid),summarize,max = mean(stemheight,na.rm=T))
