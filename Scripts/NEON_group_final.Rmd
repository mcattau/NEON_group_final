...
# there are 11 species in this plot
length(unique(as.factor(sampled_points$SOAP_ID)))

# Here is a confusion matrix that shows how confused the classes are with one another
print(fit1)

# estimate of  error rate is 37.88%
# This shows you error per species
plot(fit1)
...
## Now use party package to visualize tree

```{r visualize-trees}

set.seed(555)

# rerun random forest using cforest function from party package
temp <- cforest(as.factor(sampled_points$SOAP_ID) ~ SOAP_NDLI + SOAP_lidarSlope + nm_382 + SOAP_NDNI + nm_387 + SOAP_SAVI, data=sampled_points,
        controls=cforest_control(mtry=2, mincriterion=0))

# run script from online to make tree diagram
cforest
pt <- party:::prettytree(temp@ensemble[[1]], names(temp@data@get("input"), ilevels=temp@data[[1]]))

temp2 <- pre


temp@data@formula$input[[2]]
str(temp@data)

pt <- party:::prettytree(temp@ensemble[[1]], names(temp@data@get("input")))
pt
nt <- new("BinaryTree")
nt@tree <- pt
nt@data <- temp@data
nt@responses <- temp@responses
nt
plot(nt) 

# now we have to prune the tree

## try rpart
library(rpart)
?rpart
temp2 <- rpart(as.factor(sampled_points$SOAP_ID) ~ ., 
               data=sampled_points,
               method="class") 
printcp(temp2)
plot(temp2)
